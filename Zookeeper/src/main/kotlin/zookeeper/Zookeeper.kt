...
fun main() {
    ...
    while (input != "exit") {
        val habitat = parseHabitatNumber(input)
        printAnimal(habitat)
        input = getUserInput()
    }
    printGoodbye()
...
private fun parseHabitatNumber(input: String) =
    try {
        input.toInt()
...
private fun printAnimal(habitat: Int) {
    if (isValidHabitat(habitat)) {
        val animal = animals[habitat]
        println(animal)
    } else {
...
private fun printGoodbye() = println("See you later!")

private fun isValidHabitat(habitat: Int) = habitat != INVALID_INPUT && habitat in animals.indices